...
# ---------------------------------------------------------------------------
# Color palette (ColorBrewer RdBu diverging — colorblind-safe, print-friendly)
# ---------------------------------------------------------------------------
CLASS_COLORS = {
    0: "#2166AC",  # Enter — deep blue
    1: "#B2182B",  # Leave — deep red
    2: "#878787",  # None  — neutral gray
}
        ...
        for cls in sorted(CLASS_COLORS):
            ...
            if m.any():
                ax.scatter(
                    ...
                    s=45, alpha=0.75, edgecolors="white", linewidths=0.3,
                )
...
def fig2_collision_analysis(Z_109, y_109, event_ts_109, output_dir: Path):
    ...
    for cls in sorted(CLASS_COLORS):
        ...
        if m.any():
            ax.scatter(
                ...
                label=CLASS_NAMES[cls], s=25, alpha=0.25, edgecolors="none",
            )

    # Foreground: collision samples (prominent diamond markers)
    for cls in sorted(CLASS_COLORS):
        ...
        if m_cls.any():
            ax.scatter(
                ...
                s=160, alpha=1.0, edgecolors="black", linewidths=2.0,
                zorder=10, marker="D",
    ...
    for a, b in pairs:
        ax.plot(
            ...
            color="black", linewidth=1.8, linestyle="--", alpha=0.7, zorder=9,
        )
        ...
        ax.annotate(
            ...
            bbox=dict(
                boxstyle="round,pad=0.25", fc="#FFFFCC",
                ec="#888888", alpha=0.9,
            ),
        )
    ...
    ax.legend(
        ...
        edgecolor="#CCCCCC", markerscale=0.7,
    )
    fig.tight_layout()
...
def fig3_classification_overlay(Z, y, y_pred_rf, y_pred_mlp, output_dir: Path):
    ...
    for col, (name, y_pred, accent) in enumerate([
        ("Random Forest (sklearn)", y_pred_rf, MODEL_BLUE),
        ("MLP[64]-d0.5 (neural)", y_pred_mlp, MODEL_RED),
    ]):
        ax = axes[col]
        correct = y == y_pred
        incorrect = ~correct

        # Correct samples: colored by true class, filled circles
        for cls in sorted(CLASS_COLORS):
            m = (y == cls) & correct
            if m.any():
                ax.scatter(
        ...
        # Misclassified samples: X markers, prominent
        if incorrect.any():
            ax.scatter(
                emb[incorrect, 0], emb[incorrect, 1],
                c=[CLASS_COLORS[int(c)] for c in y[incorrect]],
                s=120, alpha=1.0, edgecolors="#222222", linewidths=2.0,
            ...
            for idx in np.where(incorrect)[0]:
                ...
                ax.annotate(
                    ...
                    bbox=dict(
                        ...
                        ec="#222222", alpha=0.9, linewidth=0.8,
                    ),
                )
        ...
        ax.set_title(
            f"{name}\nAccuracy = {acc:.2f}%  ({n_err} error{'s' if n_err != 1 else ''})",
            fontsize=11, fontweight="bold",
        )
        ax.set_xlabel("t-SNE 1")
        ax.set_ylabel("t-SNE 2")
...
def fig4_uncertainty_analysis(
    ...
    sc = ax.scatter(
        ...
        s=35, alpha=0.85, vmin=0, vmax=max_ent, edgecolors="white",
        linewidths=0.2,
    )
    cbar = fig.colorbar(sc, ax=ax, shrink=0.8, pad=0.02)
    cbar.set_label("Entropy", fontsize=9)
    ax.set_title("(a) RF — Prediction Entropy", fontsize=11, fontweight="bold")
    ax.set_xlabel("t-SNE 1")
    ax.set_ylabel("t-SNE 2")
    ...
    cbar = fig.colorbar(sc, ax=ax, shrink=0.8, pad=0.02)
    cbar.set_label("Entropy", fontsize=9)
    ax.set_title("(b) MLP — Prediction Entropy", fontsize=11, fontweight="bold")
    ax.set_xlabel("t-SNE 1")
    ax.set_ylabel("t-SNE 2")
    ...
    if correct_both.any():
        ax.scatter(
            ...
            c="#AAAAAA", s=20, alpha=0.4, label="Both correct",
        )
    if wrong_both.any():
        ax.scatter(
            ent_rf[wrong_both], ent_mlp[wrong_both],
            c="#B2182B", s=100, alpha=1.0, marker="X",
    ...
    if rf_only_wrong.any():
        ax.scatter(
            ...
            c=MODEL_BLUE, s=70, alpha=0.9, marker="s",
    ...
    if mlp_only_wrong.any():
        ax.scatter(
            ...
            c=MODEL_RED, s=70, alpha=0.9, marker="^",
            edgecolors="#222222", linewidths=0.5,
    ...
    ax.set_title(
        "(c) Per-Sample Entropy: RF vs MLP", fontsize=11, fontweight="bold",
    )
    ...
    # Color by correctness (using RF result as reference)
    colors = np.where(y == y_pred_rf, "#4393C3", "#B2182B")
    ax.scatter(conf_rf, conf_mlp, c=colors, s=25, alpha=0.6)
    ax.plot([0.3, 1], [0.3, 1], "k--", alpha=0.25, linewidth=1)
    ax.set_xlabel("RF Max Probability")
    ax.set_ylabel("MLP Max Probability")
    ax.set_title(
        "(d) Prediction Confidence", fontsize=11, fontweight="bold",
    )
    ...
    ax.legend(
        handles=legend_elems, fontsize=8,
        frameon=True, framealpha=0.9, edgecolor="#CCCCCC",
    )

    fig.suptitle(
        "Uncertainty & Confidence Analysis: RF vs MLP (N=105, L3, M+C, 2+1+2)",
